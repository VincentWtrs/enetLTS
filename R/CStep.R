CStep <- function(x, y, family, indx, h, hsize, alpha, lambda, scal){
  ...
  if (scal) {
    ...
    if (family == "binomial") {
      ...
      print(paste("The amount of data here is:", nrow(x[indx, ]))) # TO DO: remove
      ...
      resid <- -(ys * cbind(1, x) %*% beta_with_int) + log(1 + exp(cbind(1, x) %*% beta_with_int)) # NEW: used beta_with_int and cbind(1, Xs) to accomodate
