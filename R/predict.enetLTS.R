predict.enetLTS <- function(object, newX, vers = c("reweighted", "raw", "both"),
      ...
      type <- match.arg(type)
      vers <- match.arg(vers)
      if(type == "class" & object$inputs$family != "binomial"){ 
         ...
         if (object$inputs$family == "binomial"){
            if (type == "class"){
               if (vers == "reweighted"){
                  ...
                  cnum <- ifelse(test = res > 0.5, yes = 2, no = 1)
                  ...
                  cnum <- ifelse(test = res1 > 0.5,yes = 2, no = 1)
                  reweighted.class=object$classnames[cnum]
                  res2 <- newX%*%raw.coefficients
                  cnum <- ifelse(test = res2 > 0.5, yes = 2, no = 1)
                  raw.class <- object$classnames[cnum]
